...
def showcommand_ce(efile,output):
    ...
    for i in data.index:
        print("!")
        print("show bgp vrf " + data['B(vrf)'][i] + " summary")
        print("show bgp vrf " + data['B(vrf)'][i] + " neighbor " + data['A(IP)'][i] + " routes")
        print("show bgp vrf " + data['B(vrf)'][i] + " neighbor " + data['A(IP)'][i] + " advertised-routes")
        print("show bgp vrf " + data['B(vrf)'][i] + " neighbor " + data['A(IP)'][i] + " advertised-count")
...
def bgp_pe(efile,output):
    ...
    for i in data.index:
        ...
        print("     password clear C3lc@m_POI")
        print("!")
        
def bgp_ce_xr(efile,output):
    excel_data = pd.read_excel(efile)
    data = pd.DataFrame(excel_data, columns=['A(IP)','A(vrf)','B(IP)', 'B(vrf)'])
    ...
    for i in data.index:
        print("vrf " + data['B(vrf)'][i])
        print("   neighbor  " + data['A(IP)'][i])
        print("     password clear C3lc@m_POI")
        print("!")


output = 'showoutput'
efile = 'BGP_PE_CE.xlsx'
sys.stdout = open(output,"w")
showcommand_pe(efile,output)
showcommand_ce(efile,output)
bgp_pe(efile,output)
bgp_ce_xr(efile,output)